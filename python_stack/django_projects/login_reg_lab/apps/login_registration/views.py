...
def process_registration(request):
    ...
    if request.method == "POST":
        ...
        i = Account.objects.filter(email=email).count()
        if (i != 0):
        ...
        if bFlashMessage:
            ...
            Account.objects.create(first_name=f_name, last_name=l_name, email=email, pwhashval=pwhash.decode())
...
def process_login(request):
    ...
    if request.method == "POST":
        ...
        obj = Account.objects.filter(email=loginemail)
        i = obj.count()
...
def runonce(request):
    ...
    print(Account.objects.create(first_name="Foghorn", last_name="Leghorn", email="rooster@farm.com", pwhashval=hash1.decode()))
